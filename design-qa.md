...
- 展开态顶部使用排行图标、主标题和副文案，布局参考“我的”模块。
- 榜单标题行左侧使用“好友排行”，右侧使用“近 8 周”。
- 固定态只保留排行图标、“好友排行”和“近 8 周”。
- 删除 `FRIENDS / RANKING`、右侧装饰性大图标和头部分割线。
- 榜单主体不再使用“综合排名”标题。
- 排名算法、榜单数据、前三名颜色、空状态和 `70% / 30%` 计算说明不修改。

## 共享元素与滚动

- `RankingSharedTransition` 使用源元素、不可见目标槽位和唯一共享视觉层。
- 三个共享元素由 `.app-content` 的真实 `scrollTop` 连续驱动，可正向和反向播放。
- 移动端折叠距离 `132px`，桌面端 `150px`，固定阈值为 `0.72`。
- 移动端固定栏高度 `54px`，桌面端 `56px`，图标目标尺寸 `26px`。
- 页面入场完成前保持源元素可见；字体加载、窗口尺寸和元素尺寸变化后重新测量。
- 滚动帧内不重复读取布局，只通过一个 `requestAnimationFrame` 写入状态。
- 排行页不监听 `touchmove` 或 `wheel`，不增加吸附、橡皮筋和手势拦截。

## 响应式与状态

- `390×844`、`375×812` 和 `320px` 窄屏下，左右边距按 `24px` 对齐。
- 移动端展开头部高度收紧为 `104px`，榜单区域顶部间距为 `16px`。
- “近 8 周”在好友排行标题的 `24px` 行盒内垂直居中。
- “好友排行”允许在极窄宽度省略，“近 8 周”必须完整显示。
- 桌面端继续使用排行榜和计算说明双栏布局。
- 空榜单时顶部身份结构保持不变，不生成演示排名。
- `prefers-reduced-motion` 在阈值前后直接切换展开态和固定态。
- 切换离开排行时恢复源元素并清理 CSS 变量、data 属性、监听器、观察器和动画帧。

## 自动化验证

- `tests/rendered-html.test.mjs` 覆盖排行专用结构、共享组件、滚动变量、固定阈值、响应式尺寸和禁止的手势逻辑。
- TypeScript、ESLint、定向测试、生产构建、完整测试和 `git diff --check` 均已通过。
- 定向测试 `17 / 17` 通过，完整测试 `46 / 46` 通过。

## 用户设备验收

- 真机确认慢速、快速、惯性和反向滚动是否跟手。
- 真机确认固定栏最终位置与今日、计划、我的一致。
- 真机确认榜单标题和第一行经过固定背景时不重叠。
- 真机确认四个底部模块来回切换后，排行始终从完整展开状态进入。

final result: automated checks passed; awaiting user device review
